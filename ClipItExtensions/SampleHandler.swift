...
final class SampleHandler: RPBroadcastSampleHandler {
    ...
    override func processSampleBuffer(
        ...
        guard
            sampleBufferType == .video || sampleBufferType == .audioApp
        ...
        do {
            ...
            for segment in selectedSegments {
                ...
                if assetAudioTracks.count > 1,
                    let micTrack = assetAudioTracks.last, let micAudioTrack
                {
    ...
    private func axisAlignedRenderSize(
        ...
        let r = CGRect(origin: .zero, size: naturalSize).applying(
            layerTransform
        )
    ...
    private func exportLayerTransform(
        ...
        func apply(_ extra: CGAffineTransform) -> (
            CGAffineTransform, CGSize, Bool
        ) {
            let T = extra.concatenating(P)
            return (
                T,
                axisAlignedRenderSize(
                    naturalSize: naturalSize,
                    layerTransform: T
                ), true
            )
        ...
        let needsLandscapeFix =
            broadcastCaptureIsLandscape
            && baselineSize.width < baselineSize.height
    ...
    private func makeExportVideoComposition(
        ...
        let orientationRaw =
            stateService.getBroadcastCaptureInterfaceOrientationRaw()
        let captureOrientation =
            UIInterfaceOrientation(rawValue: orientationRaw) ?? .unknown
        let (layerTransform, renderSize, appliedOrientationFix) =
            exportLayerTransform(
                naturalSize: naturalSize,
        ...
        if isProUser {
            if stateService.isProWatermarkEnabled(),
                let logo = loadUserLogoImage()
            {
                wantsWatermark = true
                watermarkImage = logo
