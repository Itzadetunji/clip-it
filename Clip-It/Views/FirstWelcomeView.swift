...
struct FirstWelcomeView: View {
  ...
  private let steps: [WelcomeStep] = [
    WelcomeStep(
      ...
      description: "Gaming and you just one shotted someone but forgot to record? Use Clip It."
    ),
    WelcomeStep(
      icon: "plus.circle.fill",
      title: "Add to Control Centre",
      description: "Add Clip It to your Control Centre so you can start recording anytime."
    ),
    WelcomeStep(
      icon: "record.circle.fill",
      title: "Record & Clip",
      description: "Press Record in the app. Once you hit your moment, press the Clip button in the Control Centre."
    ),
  ]
  ...
  private var stepIndicator: some View {
    HStack(spacing: 8) {
      ForEach(0..<steps.count, id: \.self) { index in
        Circle()
          .fill(index == currentStep ? primaryColor : Color(.systemGray4))
